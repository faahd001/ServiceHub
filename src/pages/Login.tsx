...
import { Hammer, Github, Mail, Lock, AlertCircle, ExternalLink, Smartphone, ArrowLeft } from 'lucide-react';
...
export default function Login() {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      if (userData?.twoFactorEnabled) {
        setTempUser(user);
        setShow2FA(true);
        // We don't sign out yet, but we don't navigate either
      } else {
        navigate('/dashboard');
      }
    } catch (err: any) {
  ...
  const handleVerify2FA = async (e: FormEvent) => {
    ...
    try {
      ...
      if (twoFactorCode === '123456' || twoFactorCode.length === 6) { // Accept any 6-digit code for demo
        navigate('/dashboard');
      } else {
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      ...
      >
        <div className="text-center mb-10">
          ...
          <p className="text-gray-500 mt-2">Sign in to your ConnectPro account</p>
        </div>
            ...
            >
              ...
              <form onSubmit={handleVerify2FA} className="space-y-6">
                <Input 
                  ...
                  autoFocus
                />
                <Button type="submit" className="w-full h-12 rounded-xl text-lg" isLoading={loading} disabled={twoFactorCode.length !== 6}>
                  Verify & Sign In
                </Button>
                <button 
                  type="button"
